    ...
    class Column < ApplicationComponent
      ...
      def cell
        if @attribute.to_s == Book.primary_attribute.to_s
          th(scope: "row",
             class: tokens("cursor-pointer text-sm font-medium text-gray-900 text-left sticky left-[calc(3rem+1px)] row-group-has-checked:text-blue-900",
          ...
          td(class: tokens("text-sm text-gray-500 cursor-pointer whitespace-nowrap text-ellipsis overflow-hidden",
      ...
      def body
        ...
        div(class: tokens("inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium", tailwind_color_for_enum)) { @record.public_send(@attribute).to_s }
      end
